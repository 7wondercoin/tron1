...
contract TrxChain {
    ...
    uint8 public daily_returns = 133;
    uint8 public divisor = 100;
    ...
    constructor(address payable _owner) public {
        ...
        ref_bonuses.push(8);
        ref_bonuses.push(8);
        ref_bonuses.push(8); // 130
        ref_bonuses.push(5);
        ref_bonuses.push(5);
        ref_bonuses.push(5);
        ref_bonuses.push(5);
        ref_bonuses.push(5); // 155
    ...
    function _setUpline(address _addr, address _upline) private {
        if(users[_addr].upline == address(0) && _upline != _addr && _addr != owner && (users[_upline].deposit_time > 0 || _upline == owner)) {
            ...
            for(uint8 i = 0; i < ref_bonuses.length; i++) {
                ...
                users[_upline].total_team_business += msg.value;
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * daily_returns * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 100) - users[_addr].deposit_payouts;
